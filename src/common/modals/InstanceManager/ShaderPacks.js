      ...
      >
        ...
        <div className="rightPartContent">
          ...
          <TrashIcon
            selectedMods
            onClick={() => {
              deleteFile(item, instancesPath, slcItems, sdPacksPath, name);
            }}
            icon={faTrash}
          />
        </div>
    ...
    >
        ...
        >
          ...
          <OpenFolderButton
            onClick={async () => {
              ...
              openFolder(path.join(instancesPath, instanceName, 'shaderpacks'));
            }}
            icon={faFolder}
          />
        </div>
      ...
      >
        {shaderPacks.length === 0 && (
          <NotItemsAvailable>シェーダーパックが無いギリ</NotItemsAvailable>
        )}
        <AutoSizer>
          {({ height, width }) => (
            <List
